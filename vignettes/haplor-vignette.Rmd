...
_LDlink_ is a set of web-based tools that allow exloring Linkage Disequilibrium of provided set of SNPs. _LDlink_ allows for the following tools:
...
Currently _haploR_ support _LDmatrix_ only but we are currently working on adding other tools from _LDlink_ project.
...
* _r2d_:	Show r2 or D'. Default: r2
* _population_:	A particular population. Default: ```ALL```.

##### Value

A list of three: (1) raw LD r2 matrix, (2) colored (fancy) matrix with LD r2 gradient (an object of classses _datatables_, _htmlwidget_), (3) raw LD D-prime matrix.
...
# Raw LD matrix r2:
head(ldmat$raw.matrix.r2)
# Raw LD matrix D prime:
head(ldmat$raw.matrix.dprime)
# Fancy-looking LD matrix r2:
ldmat$stylish.matrix.r2
```
